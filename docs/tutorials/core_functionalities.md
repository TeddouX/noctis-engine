...
As you've probably already seen, the Noctis Engine uses a custom made logger to print messages to the console. You can use it yourself in your project with the [Logger](@ref NoctisEngine::Logger) class defined in the noctis_engine/core/logging.hpp header:
...
auto main() -> int
{
    NoctisEngine::Logger my_logger{"My Game", "Main"};
...
Note that [.debug()](@ref NoctisEngine::Logger::debug()) only prints its message to the screen if the macro NCENG_DEBUG is defined. You can enable and disable this macro's definition in your CMakeLists.txt:
...
Input is handled by the [InputHandler](@ref NoctisEngine::InputHandler) singleton (meaning a class that doesn't need to be instantiated), which gets updated when you call [window.poll_events()](@ref NoctisEngine::Window::poll_events()). Preferably poll events before anything else in your update loop.  
There are 3 ways of handling input using Noctis Engine, each with their own advantages.  
The first way is using the [InputHandler](@ref NoctisEngine::InputHandler) class directly. It can be included from the input_handler.hpp header. This is the most basic one and the easiest to understand:
...
auto main() -> int
{
    NoctisEngine::Window window{800, 600, "Hello window"};
    NoctisEngine::Logger my_logger{"Docs", "Main"};
    ...
    while (not window.should_close())
    {
        ...
        if (NoctisEngine::InputHandler::key_just_pressed(NoctisEngine::PhysicalKey::SPACE))
...
If you run this code and press the spacebar, you should see the log message printed only once. Go see the [InputHandler](@ref NoctisEngine::InputHandler) documentation to see all the other input functions.  
@warning [PhysicalKey](@ref NoctisEngine::PhysicalKey) uses the QWERTY keyboard layout.   

The second way of handling input is using the [InputHandler](@ref NoctisEngine::InputHandler) with actions instead of raw keycodes:  
...
auto main() -> int
{
    ...
    while (not window.should_close())
    {
        ...
        if (NoctisEngine::InputHandler::action_just_pressed(PLAYER_JUMP_ACTION))
...
If you run this code and press the the A key (QWERTY layout) or the left mouse button, you should see the log message printed only once. Go see the [InputHandler](@ref NoctisEngine::InputHandler) documentation to see all the other action functions.  
The last and more complicated way to handle input is using the the [EventStack](@ref NoctisEngine::EventStack). This is mainly useful for UIs:
...
class MyUI : public NoctisEngine::IEventReceiver
{
public:
    ...
    auto on_keyboard_event(const NoctisEngine::KeyboardInputEvent &event) -> bool override
    {
        my_logger.info("some info: you did something with the key ordinal {}", NoctisEngine::ordinal(event.key));

        // Only the pressed and released states work in these callback functions
        if (event.info.state == NoctisEngine::InputInfo::State::PRESSED 
            && event.key == NoctisEngine::PhysicalKey::Q)
        {
...
auto main() -> int
{
    ...
    NoctisEngine::EventStack::push_receiver(my_ui);
    ...
    NoctisEngine::EventStack::pop_receiver();
