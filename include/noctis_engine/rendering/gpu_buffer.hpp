...
enum class BufferFlag : std::uint32_t 
{
    ...
    /// @warning requires MAP_READ_BIT or MAP_WRITE_BIT to be set
    ...
    /// @warning Requires MAP_PERSISTENT_BIT to be set
...
enum class BufferMapAccess : std::uint32_t 
{
    ...
    /// @warning DO NOT USE WHEN MAPPING BUFFERS
    NONE = 0,
    ...
    /// @warning Requires MAP_READ_BIT to not be set
    ...
    /// @warning Requires MAP_WRITE_BIT to be set
...
public:
    ...
    /// @warning This buffer must've created with either the MAP_READ_BIT flag or the MAP_WRITE_BIT flag 
    ...
    /// @brief Writes data to this buffer
    /// @param data The data that should be written
    /// @param offset The offset inside the buffer the data should be written at
    /// @warning This buffer must've created with the GL_DYNAMIC_STORAGE flag for this function 
    ...
    /// @warning The buffer must have been mapped with the MAP_FLUSH_EXPLICIT_BIT flag for this 
    /// function to not cause an OpenGL error.
    ...
    /// @brief Get this buffer's size in bytes
    auto size() const -> std::size_t;

    /// @brief Get this  buffer's creation flags 
    auto flags() const -> BufferFlag;

    /// @brief Get this  buffer's map accesses 
    auto map_access() const -> BufferMapAccess;

    /// @brief Get this  buffer's OpenGL handle 
    auto gl_handle() const -> std::uint32_t;

    /// @brief Get this  buffer's name
